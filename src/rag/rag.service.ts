...
function resolveCta(sources: RagDocumentWithScore[]): Cta | null {
  ...
  if (tagged) {
    return {
      label: tagged.metadata.linkLabel ?? tagged.title,
      url: tagged.metadata.link,
    }
  }
...
class ConversationMemory {
  ...
  async addTurn(
    ...
    await this.redis
      .getClient()
      .set(this.key(sessionId), JSON.stringify(turns), {
    ...
    try {
      ...
      const rows = await this.prisma.$queryRawUnsafe<Array<{ id: string }>>(
        `INSERT INTO rag_documents
          (title, content, metadata, embedding, updated_at)
         VALUES
          ($1, $2, $3::jsonb, $4::vector, now())
         RETURNING id`,
        dto.title,
        dto.content,
      ...
      if (!rows || rows.length === 0 || !rows[0].id) {
        return {
          success: false,
          title: dto.title,
          message: 'Insert returned no ID — document not saved',
        }
      }
...
{"language":"<original language and script>","rewrittenQuery":"<clear English query>"}`
  ...
  async searchSimilar(
    ...
    >(
      `SELECT
       ...
       WHERE cardinality($3::text[]) = 0
          OR NOT (
            COALESCE(metadata->>'category', '') = ANY($3::text[])
          )
    ...
    this.logger.log(
      `[searchSimilar] PostgreSQL search took ${
    ...
    this.logger.log(
      `[chat] retrieved ${sources.length} chunk(s): ` +
        sources
          .map(
            (s) =>
              `"${s.title}" (similarity: ${s.similarity.toFixed(3)})`,
          )
          .join(', '),
    )
...
- About ShoutlyAI?
  ...
  async listDocuments(opts: {
    ...
    const rows =
      await this.prisma.$queryRawUnsafe<
        Array<RagDocument & { metadata: string }>
      >(
        search
          ? `SELECT
               id::text,
  ...
  async updateDocument(
    ...
    return {
      ...
      title: newTitle,
      message:
        'Document updated and embedding refreshed',
    }
  }
